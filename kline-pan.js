(() => {
  ...
  function drawScrollableKline(items) {
    ...
    ctx.fillText(String(high.toFixed(2)).replace(/\.00$/, ""), 6, padT + 8);
    ctx.fillText(String(low.toFixed(2)).replace(/\.00$/, ""), 6, padT + priceH);
  ...
  const oldDraw = window.drawKline;
  ...
  function bind() {
    ...
    canvas.addEventListener("pointermove", (event) => {
      if (!dragging || !lastItems.length) return;
