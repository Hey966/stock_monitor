(() => {
  ...
  function calcVwap(bars) {
    ...
    bars.forEach((b) => {
      ...
      pv += ((num(b.high) + num(b.low) + num(b.close)) / 3) * v;
      vol += v;
    });
  ...
  function analyzeKline(quote, bars) {
    ...
    if (close >= mid) { score += 15; good.push('站上均價'); }
    else { score -= 18; risk.push('跌破均價'); }
    ...
    if (support && num(last.low) <= Math.max(mid, support) * 1.004 && close > Math.max(mid, support)) {
      score += 18;
      good.push('回測不破');
      pattern.push('回測不破');
    }

    if (num(last.close) > num(prev.high) && num(last.close) >= num(last.open)) {
      score += 12;
      good.push('紅K過前高');
      pattern.push('多方續攻');
    }

    if (strongVol && num(quote.change_rate) > 0.3 && upper / body < 1.35) {
      score += 10;
      good.push('放量推進');
      pattern.push('放量推進');
    }

    if (strongVol && num(quote.change_rate) <= 0.3) {
      score -= 18;
      risk.push('量增價不動');
      pattern.push('量價背離');
    }

    if (upper > body * 1.35 && upper / range > 0.35 && close >= (pressure || close) * 0.995) {
      score -= 22;
      risk.push('長上影靠近壓力');
      pattern.push('假突破');
    }

    if (num(last.close) < num(prev.low)) {
      score -= 18;
      risk.push('跌破前低');
      pattern.push('轉弱K');
    }
  ...
  function renderDecision(d) {
    ...
    const rows = [
      ['現在狀態', d.status, `分數 ${d.score}/100`],
      ['K線型態', d.pattern.join(' / ') || '等待型態', d.good[0] || '等待確認'],
      ['止損時機', price(d.riskLine), '跌破風險線需保守'],
      ['第一賣點', price(d.firstTarget), '接近壓力先留意']
    ];
    $$('.modern-cards article').forEach((card, i) => {
      if (!card || !rows[i]) return;
      card.querySelector('span').textContent = rows[i][0];
      card.querySelector('strong').textContent = rows[i][1];
      card.querySelector('p').textContent = rows[i][2];
    });

    const ids = [
      ['aiEntry', price(d.observe)], ['aiStop', price(d.riskLine)], ['aiTarget', price(d.firstTarget)],
      ['planEntry', price(d.observe)], ['planStop', price(d.riskLine)], ['planTarget', price(d.firstTarget)]
    ];
    ids.forEach(([id, text]) => { const el = $('#' + id); if (el) el.textContent = text; });

    const texts = [
      ['aiEntryText', `觀察線 ${price(d.observe)}，重點是回測不破`],
      ['aiStopText', `跌破 ${price(d.riskLine)} 代表短線轉弱`],
      ['aiTargetText', `第一壓力 ${price(d.firstTarget)}，接近先留意`],
      ['planEntryText', `觀察線 ${price(d.observe)}，重點是回測不破`],
      ['planStopText', `跌破 ${price(d.riskLine)} 代表短線轉弱`],
      ['planTargetText', `第一壓力 ${price(d.firstTarget)}，接近先留意`]
    ];
    texts.forEach(([id, text]) => { const el = $('#' + id); if (el) el.textContent = text; });
  }

  async function update(code) {
    const [quote, kbars] = await Promise.all([
      getJSON(`${API}/api/quote?code=${code}`),
      getJSON(`${API}/api/kbars?code=${code}&days=5`)
    ]);
  ...
  setInterval(() => {
    ...
    lastCode = code;
    setTimeout(() => update(code).catch(() => {}), 800);
  }, 1200);
})();
