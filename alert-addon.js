(() => {
  ...
  function render() {
    ...
    if (/回測不破|突破|VWAP|量增/.test(sig)) tags.push('技術轉強');
    const p = $('#alertPanel');
  ...
  document.addEventListener('click', e => { if (e.target && (e.target.id === 'proSearch' || e.target.id === 'marketRefresh')) setTimeout(render, 2600); }, true);
})();
